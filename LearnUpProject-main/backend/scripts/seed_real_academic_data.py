...
def _ensure_allowed_departments(session: Session) -> tuple[Faculty, Department, dict[str, int]]:
    faculty = (
    ...
    return faculty, departments_by_id[4], stats


def _ensure_computer_science_org(session: Session) -> tuple[Faculty, Department]:
    faculty, department, _stats = _ensure_allowed_departments(session)
...
def seed_real_academic_data(session: Session) -> dict[str, Any]:
    ...
    faculty, department, department_stats = _ensure_allowed_departments(session)
...
def print_summary(
    ...
    print(
        "  Departments count: "
        f"{seeded['departments']['total']} "
        f"(created {seeded['departments']['created']}, "
        f"updated {seeded['departments']['updated']}, "
        f"deleted extra {seeded['departments']['deleted_extra']})"
    )
    print(
        "  Department remaps: "
        f"students {seeded['departments']['remapped_students']}, "
        f"instructors {seeded['departments']['remapped_instructors']}, "
        f"courses {seeded['departments']['remapped_courses']}"
    )
    print(
