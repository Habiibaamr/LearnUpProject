...
type Tab = "all" | "available" | "enrolled" | "locked" | "passed";

function badgeClasses(status: instructorApi.CourseBoardStatus) {
...
export default function InstructorStudentCourseBoard() {
  const { uid } = useInstructorStudentUid();
  const [rows, setRows] = useState<instructorApi.CourseBoardRow[]>([]);
  ...
  const loadBoard = async () => {
    ...
    setErr("");
    try {
      setRows(await instructorApi.getStudentCourseBoard(uid));
    } catch (e) {
  ...
  const runAction = async (row: instructorApi.CourseBoardRow, action: "add" | "drop") => {
    if (!uid) return;
    setMsg("");
    setErr("");
    setBusyCourseId(row.course_id);
    try {
      if (action === "add") await instructorApi.addCourseForStudentByCourse(uid, row.course_id);
      else await instructorApi.dropCourseForStudentByCourse(uid, row.course_id);
      setMsg(action === "add" ? "Course enrolled for student." : "Course dropped for student.");
      await loadBoard();
    } catch (e) {
      setErr(getApiErrorMessage(e));
    } finally {
  ...
  if (!uid) {
    return (
      <FeaturePageLayout title="Student course board" subtitle="Add or drop courses for a student">
  ...
  return (
    <FeaturePageLayout
      ...
      subtitle={`Same rules as the student portal (level, prerequisites, credit limits). Student: ${uid}`}
    >
          ...
          {filtered.map((row) => (
            <div key={row.course_id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{row.course_code}</p>
